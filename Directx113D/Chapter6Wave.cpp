#include "stdafx.h"
#include "Chapter6Wave.h"
#include "d3dUtil.h"

#include <d3dcompiler.h>
#pragma comment (lib, "D3DCompiler.lib")

Chapter6Wave::Chapter6Wave() 
	: mLandVB(0), mLandIB(0), mWavesVB(0), mWavesIB(0), mFX(0), mTech(0),
	mfxWorldViewProj(0), mInputLayout(0), mWireframeRS(0), mGridIndexCount(0),
	mTheta(1.5f*XM_PI), mPhi(0.25f*XM_PI), mRadius(5.0f),
	px(0), py(0), pz(0)
{
	mTitleCaption = L"Waves Demo";
...
void Chapter6Wave::InitD3D(HWND hWnd)
{
...
void Chapter6Wave::Update(float dt)
{
	...
	VertexC* v = reinterpret_cast<VertexC*>(mappedData.pData);
...
void Chapter6Wave::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void Chapter6Wave::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void Chapter6Wave::OnMouseMove(WPARAM btnState, int x, int y)
{
...
float Chapter6Wave::GetHeight(float x, float z) const
{
	return 0.3f*(z*sinf(0.1f*x) + x * cosf(0.1f*z));
}

void Chapter6Wave::BuildLandGeometryBuffers()
{
...
void Chapter6Wave::BuildWavesGeometryBuffers()
{
	...
	vbd.ByteWidth = sizeof(VertexC) * mWaves.VertexCount();
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
...
void Chapter6Wave::BuildVertexLayout()
{
